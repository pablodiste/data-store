...
interface StalenessPolicy<K: Any, T: Any> {
    //fun removeStaleEntity(cache: RoomCache<K, T>, key: K, entity: T)
    fun removeStaleList(cache: RoomListCache<K, T>, key: K, entity: List<T>)
}
...
class DoNotExpireStalenessPolicy<K: Any, T: Any>: StalenessPolicy<K, T> {
    override fun removeStaleList(cache: RoomListCache<K, T>, key: K, entity: List<T>) {
    }
}
